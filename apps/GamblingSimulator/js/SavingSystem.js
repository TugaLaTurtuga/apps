...
function downloadGameData() {
    ...
    // Convert gameData (string) to a encoded in base 85 (Uint8Array)
    const byteArray = new TextEncoder().encode(gameData);
    const base85Encoded = encrypt(byteArray);

    // Create a ZIP file and add the Base85 encoded data
    const zip = new JSZip();
    zip.file("gameData.txt", base85Encoded); // Store the encoded game data as a text file
